"""Universe selector: picks the best "flex" tickers from the candidate pool.
...
Process:
  ...
  6. Apply sector-diversity caps and pick top --n (default 28).
  7. Write the final ({n_required}+--n)-stock universe to config/universe.py.

The default --n is 28, which combined with 7 required tickers produces
a 35-stock KSE-100-mirroring universe.
...
Usage:
    ...
    python scripts/select_universe.py --n 28       # number of flex slots to pick
"""
...
# How many stocks allowed per grouped sector in the FINAL 35.
# Mirrors KSE-100 weight distribution; tighten or relax via candidates.py.
SECTOR_CAPS: dict[str, int] = {
    "Banking":             8,   # ~25% of KSE-100 by weight
    "Oil & Gas E&P":       4,
    "OMC/Refining":        4,
    "Cement":              6,
    "Fertilizer":          3,
...
def pick_flex(
    ...
    Defaults to 28 flex slots so the final universe is 35 names
    (7 required + 28 flex). Override with --n on the CLI.
    """
    picked: list[dict] = []
...
# Back-compat alias for any external callers; new code uses pick_flex.
pick_flex_nine = pick_flex


# --------------------------------------------------------------------------
# Writing the final universe.py
# --------------------------------------------------------------------------
UNIVERSE_TEMPLATE = '''"""Trading universe: the {n_total} KSE-100 names the bot trades.
...
NAMES: dict[str, str] = {
    ...
    "NBP":  "National Bank", "AKBL": "Askari Bank",
    "MARI": "Mari Petroleum", "POL": "Pakistan Oilfields",
    "PSO":  "Pakistan State Oil", "APL": "Attock Petroleum",
    "ATRL": "Attock Refinery", "NRL": "National Refinery",
    "LUCK": "Lucky Cement", "FCCL": "Fauji Cement",
    "DGKC": "D.G. Khan Cement", "KOHC": "Kohat Cement",
    "BWCL": "Bestway Cement", "CHCC": "Cherat Cement",
    ...
    "AVN":  "Avanceon",
    "INDU": "Indus Motor", "HCAR": "Honda Atlas Cars",
    ...
    "NESTLE": "Nestle Pakistan", "UFL": "Unity Foods",
...
def write_universe(picks: list[dict], required_entries: list[dict], dry_run: bool) -> Path:
    ...
    content = UNIVERSE_TEMPLATE.format(
        ...
        n_flex=len(picks),
        n_total=len(required_entries) + len(picks),
        entries="\n".join(lines),
    )
...
def main() -> int:
    ...
    parser.add_argument("--n", type=int, default=28,
                        help="Number of flex slots to fill (default 28; "
                             "with 7 required tickers this gives a "
                             "35-stock universe)")
    ...
    # 4. Pick flex slots from pool with sector caps
    picks = pick_flex(pool_results, req_sector_counts,
                       n_slots=int(args.n))
    ...
    if args.dry_run:
        ...
        console.print("\nNext steps:")
        console.print(f"  1. python scripts/backfill.py         "
                       f"# make sure all {len(required_entries) + len(picks)} are backfilled")
